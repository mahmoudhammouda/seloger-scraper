...
interface RecordedEvent {
  at: number; // millisecondes depuis le début du record
  type: string;
  selector: string;
  value?: string;
}
...
let currentRecordName: string | null = null;
let recordStartTs = 0;
let currentRecordEvents: RecordedEvent[] = [];
let recordFinished: (() => void) | null = null;
...
const appendLine = (filePath: string, line: string, _alsoConsole = false) => {
...
const saveCurrentRecord = (): string | null => {
  if (!currentRecordName || !currentRecordEvents.length) return null;
...
const loadRecordByName = (name: string): { record: SavedRecord; file: string } | null => {
  if (!fs.existsSync(RECORDS_DIR)) return null;
  const files = fs.readdirSync(RECORDS_DIR).filter((f) => f.endsWith('.json'));
  let best: { record: SavedRecord; file: string; ts: number } | null = null;
  for (const file of files) {
    try {
      const full = path.join(RECORDS_DIR, file);
      const data = JSON.parse(fs.readFileSync(full, 'utf-8')) as SavedRecord;
      if (!data?.name || !Array.isArray(data.events)) continue;
      if (data.name.toLowerCase() !== name.toLowerCase()) continue;
...
const setProxyStatusOverlay = (page: Page | null, text: string, tone: 'ok' | 'warn' | 'error') => {
  if (!page) return;
  page
    .evaluate(
      (payload) => {
...
const performEvent = async (page: Page, ev: RecordedEvent) => {
  ...
  try {
    ...
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION record.replay.event ok type=${ev.type} selector=${ev.selector}`
    );
  } catch (err) {
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION record.replay.event failed type=${ev.type} selector=${ev.selector} err=${String(err)}`
    );
  }
};

const replayRecord = async (page: Page | null, name?: string) => {
  if (!page) {
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION record.replay.error no active page`
    );
    return;
  }
  ...
  if (!loaded) {
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION record.replay.error not_found name=${targetName}`
    );
    return;
  }
  appendLine(
    ACTION_LOG,
    `[${new Date().toISOString()}] ACTION record.replay.start name=${loaded.record.name} events=${loaded.record.events.length} file=${loaded.file}`
  );
  let lastAt = 0;
  ...
  appendLine(
    ACTION_LOG,
    `[${new Date().toISOString()}] ACTION record.replay.end name=${loaded.record.name}`
  );
};
...
const isCaptchaPage = async (page: Page): Promise<boolean> => {
  ...
  return page.evaluate(() => {
    return (
      document.body.textContent?.toLowerCase().includes('datadome') ||
...
const parseProxyLine = (line: string): ProxyConfig | null => {
...
const loadProxyPool = (proxyListPath?: string): ProxyConfig[] => {
  if (!proxyListPath || !fs.existsSync(proxyListPath)) return [];
  const lines = fs.readFileSync(proxyListPath, 'utf-8').split('\n').filter(Boolean);
  return lines
    .map(parseProxyLine)
    .filter((p): p is ProxyConfig => Boolean(p));
};

function pickProxy(config: Config): { proxy: ProxyConfig; source: string } {
...
async function logPublicIp(context: BrowserContext): Promise<boolean> {
  try {
...
async function navigateToSearch(page: Page, zoneCode: string, pageNumber: number): Promise<boolean> {
  ...
  try {
    ...
    if (response && (response.status() === 403 || response.status() === 429)) {
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] Navigation blocked status=${response.status()}`
      );
      return false;
    }
...
async function selectZoneViaUi(page: Page, zoneCode: string): Promise<boolean> {
  ...
  try {
    ...
    if (newSearchButton) {
      ...
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] ACTION ui.click new search button (Créer une nouvelle recherche)`
      );
      await randomDelay(1000, 2000);
    }
  } catch {
  ...
  for (const sel of searchSelectors) {
    try {
      ...
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] ACTION ui.click selector=${sel} to type zone`
      );
      // Dans certains cas le selecteur est un bouton, on tente de trouver le champ adjacent
      const inputHandle = await page.$(
        'input[placeholder*="Ville"], input[placeholder*="localisation"], input[placeholder*="Où"], input[type="search"], input[data-testid="search-input"]'
      );
      const targetForType = inputHandle || (await page.$(sel));
      if (!targetForType) continue;
      ...
      if (picked.clicked) {
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION ui.select suggestion="${picked.text}"`
        );
      } else {
  ...
  appendLine(
    ACTION_LOG,
    `[${new Date().toISOString()}] ACTION ui.selectZone failed: no selector matched`,
    true
  );
  return false;
}
...
async function extractListings(page: Page): Promise<string[]> {
  ...
  try {
    ...
    const urls = await page.evaluate(() => {
      ...
      cards.forEach((card) => {
        ...
        links.forEach((link) => {
          ...
          if (
            href.includes('/annonces/achat/appartement/') ||
    ...
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION extraction.done count=${urls.length}`
    );
    return urls;
  } catch (error) {
...
function validateUrlsForZone(urls: string[], zoneCode: string): { ok: boolean; ratio: number } {
  if (!urls.length) return { ok: false, ratio: 0 };
...
async function waitForManualAssistance(page: Page, reason: string, ms = 30000) {
  appendLine(
    ACTION_LOG,
    `[${new Date().toISOString()}] ACTION manual.assist.start reason=${reason} waitMs=${ms}`
  );
  try {
    ...
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION manual.assist.screenshot path=${shotPath}`
    );
  } catch {
    /* ignore */
  }
  ...
  appendLine(
    ACTION_LOG,
    `[${new Date().toISOString()}] ACTION manual.assist.end reason=${reason} elapsed=${ms}`
  );
}

async function injectRecordingUi(page: Page) {
  ...
  try {
    await page.addInitScript(() => {
      const inject = () => {
        ...
        // @ts-ignore
        proxyCheckbox.checked = (window.__initialProxyEnabled ?? true) as boolean;
        ...
        window.__overlayLog = (msg) => {
          try {
            ...
            while (content.childElementCount > 120) {
              content.removeChild(content.firstChild as Node);
            }
        ...
        window.__setProxyToggleState = (enabled) => {
          const cb = document.getElementById('__proxy_toggle') as HTMLInputElement | null;
          if (!cb) return;
          cb.checked = !!enabled;
        };
      };
...
async function handleRequestLogging(req: Request) {
  appendLine(
    NETWORK_LOG,
    `[${new Date().toISOString()}] Request ${req.method()} ${req.url()} frame=${req.frame()?.url() || 'n/a'} type=${req.resourceType()}`,
    false
  );
}

async function handleResponseLogging(res: Response, suspected: { blocked: boolean }) {
  const url = res.url();
  appendLine(
    NETWORK_LOG,
    `[${new Date().toISOString()}] Response ${res.status()} ${res.request().method()} ${url}`,
    false
  );
  ...
  if (
    ...
    appendLine(
      NETWORK_LOG,
      `[${new Date().toISOString()}] DataDome/Fraud ${res.status()} ${res.request().method()} ${url}`
    );
  }
}

async function handleRequestFailedLogging(req: Request, suspected: { blocked: boolean }) {
  appendLine(
    NETWORK_LOG,
    `[${new Date().toISOString()}] Request failed ${req.method()} ${req.url()} | ${req.failure()?.errorText || 'unknown'}`,
    false
  );
...
async function runRecorder(config: Config): Promise<{ proxyFailed: boolean }> {
  const { proxy } = config;
  ...
  try {
    ...
    await context.exposeBinding('playwrightRecordControl', async (_source, payload: any) => {
      ...
      if (action === 'start') {
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION record.start name=${currentRecordName}`,
          true
        );
      } else if (action === 'stop') {
        isRecording = false;
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION record.stop saved=${savedPath || 'none'} events=${currentRecordEvents.length}`,
          true
        );
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION record.play name=${name || 'unnamed'}`,
          true
        );
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION record.unknown action=${action}`,
          true
        );
      }
    });
    await context.exposeBinding('playwrightClearCookies', async () => {
      try {
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION clear.cookies error ${String(err)}`,
          true
        );
        setProxyStatusOverlay(mainPage, 'proxy: clear failed', 'error');
      }
    });
    await context.exposeBinding('playwrightProxyToggle', async (_source, enabled: boolean) => {
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] ACTION proxy.toggle requested=${enabled} (effet au prochain run)`,
        true
      );
      setProxyStatusOverlay(
        mainPage,
        enabled ? 'proxy ON (appliqué au prochain run)' : 'proxy OFF (appliqué au prochain run)',
        enabled ? 'warn' : 'warn'
      );
    });
    ...
    await page.addInitScript(() => {
      ...
      const forwardEvent = (type: string, ev: any) => {
      ...
      const mouseMoveHandler = throttle((ev: MouseEvent) => {
        ...
        window.logUserEvent?.({
          type: 'mousemove',
          target: (ev.target as HTMLElement)?.tagName?.toLowerCase() || 'unknown',
    ...
    await page.exposeBinding('playwrightCaptureShot', async () => {
      try {
        ...
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION screenshot.captured path=${shotPath}`,
          true
        );
      } catch (err) {
        appendLine(
          ACTION_LOG,
          `[${new Date().toISOString()}] ACTION screenshot.error ${String(err)}`,
          true
        );
      }
    });
    ...
    page.on('domcontentloaded', () =>
      appendLine(ACTION_LOG, `[${new Date().toISOString()}] Event: domcontentloaded`)
    );
    page.on('request', (req) => void handleRequestLogging(req));
    page.on('requestfinished', (req) =>
      appendLine(
        NETWORK_LOG,
        `[${new Date().toISOString()}] Request finished ${req.method()} ${req.url()}`,
        false
      )
    );
    ...
    if (proxy) {
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] Proxy actif: ${proxy.server} | user: ${proxy.username} | source: ${config.proxySource || 'config'} | headless: ${config.headless ?? false}`
      );
      setProxyStatusOverlay(
        overlayPage,
        `proxy: ${proxy.server} (${proxy.username})`,
        'warn'
      );
      ...
      if (!ipOk) proxyFailed = true;
    } else {
      appendLine(
        ACTION_LOG,
        `[${new Date().toISOString()}] Proxy désactivé (mode direct) | headless: ${config.headless ?? false}`
      );
    ...
    appendLine(
      ACTION_LOG,
      `[${new Date().toISOString()}] ACTION ready. Click Start rec to begin, Stop rec to finish.`
    );
...
async function main() {
  ...
  const rawConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8')) as Config;
